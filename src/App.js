...
import {
    View, Epic, Tabbar, ConfigProvider, Snackbar, TabbarItem
} from '@vkontakte/vkui';
...
import MeetAdmin from './components/MeetAdmin/MeetAdmin';

import AdminPage from './components/AdminPages/AdminPage';
import AddMeetPage from './components/AddMeetPage/AddMeetPage';
...
import CommIntegration from './components/Communities/CommIntegration';
...
class App extends React.Component {
    ...
    componentDidMount() {
        ...
        connect.send('VKWebAppGetAds').then(banner => {
            console.log(banner)
    ...
    render() {
        const {
            api, state, onStoryChange, getMeets, makeStory, getComments,
            getUserMeets, openErrorSnackbar, openDoneSnackbar, getOwneredMeets, getExpiredUserMeets
        } = this;

        const {
            offline, popout, activeStory, meets, userMeets, allMeets,
            activePanel, fetchedUser, comments, banner
        } = this.state;


        const isUserAdmin = [35501089, 114752686, 87478742, 236820864].includes(fetchedUser.id)
        ...
        const props = {
            ...
            comments, setParentState: this.setState.bind(this), banner
        };
