...
class AddMeetPage extends Component {
    ...
    onChange = async (e) => {
        ...
        if (name === 'file') {
            ...
            if (!(file.type.match('image/*')) || file.type.toLowerCase().match('heic')) {
                this.setState({error: true});
            ...
            <Panel id={id}>
                ...
                <Group header={<Header mode="secondary">Предпросмотр</Header>}>
                    <Div>
                        <MeetBox
                            meet={meet}
                            onAdd={state.groupSelected ? state.currentGroupInfo.name : false}
                            setParentState={null}
                ...
                <Group header={<Header mode="secondary">Информация о петиции</Header>}>
                    <FormLayout>
                        ...
                        <Input
                            ...
                            onChange={onChange}
                            status={
                                (name === undefined || name === '') && attemp ? 'error' : attemp ? 'valid' : ''
                            }
                            bottom={
                        ...
                        <Textarea
                            ...
                            onChange={onChange}
                            status={
                                (description === undefined || description === '') && attemp ? 'error' : attemp ? 'valid' : ''
                            }
                        ...
                        <Input
                            ...
                            value={start}
                            onChange={onChange}
                            status={(start === undefined || start === '' || !start) && attemp ? 'error' : (Date.parse(start) < Date.parse(new Date())) && (new Date(start).getDay() < new Date().getDay()) ? 'error' : attemp ? 'valid' : ''}
                            bottom={(start === undefined || start === '' || !start) && attemp ? 'Пожалуйста, укажите дату начала петиции' : (Date.parse(start) < Date.parse(new Date())) && (new Date(start).getDay() < new Date().getDay()) ? 'Дата начала петиции уже прошла' : ''}
                        />
                        <Input
                            type="date"
                            ...
                            value={finish}
                            onChange={onChange}
                            status={(finish === undefined || finish === '' || !finish) && attemp ? 'error' : (Date.parse(finish) < Date.parse(new Date())) && (new Date(finish).getDay() < new Date().getDay()) ? 'error' : Date.parse(start) > Date.parse(finish) ? 'error' : attemp ? 'valid' : ''}
                            bottom={(finish === undefined || finish === '' || !finish) && attemp ? 'Пожалуйста, укажите дату окончания петиции' : (Date.parse(finish) < Date.parse(new Date())) && (new Date(finish).getDay() < new Date().getDay()) ? 'Дата окончания петиции уже прошла' : Date.parse(start) > Date.parse(finish) ? 'Дата окончания петиции раньше даты начала' : ''}
                        />
                        <File
                            id='file'
                            maxfiles={1}
                            ...
                            name="file"
                            onChange={onChange}
                            status={!photo && attemp ? 'error' : attemp ? 'valid' : ''}
                        ...
                        <Checkbox
                            ...
                            status={!accept && attemp ? 'error' : attemp ? 'valid' : ''}
                        ...
                        <Button
                            ...
                            {state.disabled ? <Spinner/> : formLang.add}
                        </Button>
                    </FormLayout>
                </Group>
                {snackbar}
