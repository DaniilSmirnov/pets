...
class MeetAdmin extends Component {
    ...
    render() {
        ...
        const backgroundImage = `url(data:image/png;base64,${meet.photo})`;
            ...
            <Panel id={id}>
                ...
                <Group>
                    ...
                    <Link id='link' target="_blank" href={link}>
                        <UsersStack photos={[meet.owner_photo]}>
                            {meet.owner_name} {meet.owner_surname} • {meetMembers}
                        </UsersStack>
                    </Link>
                    <Separator style={{margin: '12px 0'}}/>
                            ...
                            !meet.approved ?
                                <Div style={{display: 'flex'}}>
                                    <Button
                                        ...
                                        style={{marginRight: 8}}
                                        onClick={approve}>
                                        {this.state.disabled ? <Spinner/> : 'Принять'}
                                    </Button>
                                    <Button
                                        size="l"
                                        disabled={this.state.disabled}
                                        ...
                                        onClick={Deny}>
                                        {this.state.disabled ? <Spinner/> : 'Отклонить'}
                                    </Button>
                                </Div>
                                : <Div style={{display: 'flex'}}>
                                    <Button
                                        ...
                                        style={{marginRight: 8}}
                                        onClick={deApprove}>
                                        {this.state.disabled ? <Spinner/> : 'Скрыть от пользователей'}
                                    </Button>
                                    <Button
                                        size="l"
                                        disabled={this.state.disabled}
